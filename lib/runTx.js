...
const log = require('loglevel').getLogger('runTx');
//log.setLevel("trace")
log.setLevel("silent")
...
module.exports = function (opts, cb) {
  ...
  function runAfterTxHook (cb) {
    log.trace('runTx.js runAfterTxHook.')
  ...
  function populateCache (cb) {
    log.trace('runTx.js populateCache...')
    ...
    log.trace('runTx.js populateCache calling warmCache..')
    self.stateManager.warmCache(accounts, cb)
  }

  // sets up the environment and runs a `call`
  function runCall (cb) {
    log.trace('runTx.js runCall...')
    // check to the sender's account to make sure it has enough wei and the correct nonce
    log.trace('runTx.js runCall calling stateManager.cache.get(tx.from)...')
    var fromAccount = self.stateManager.cache.get(tx.from)
    log.trace('runTx.js runCall stateManager.cache.get returned fromAccount:', fromAccount)
    log.trace('fromAccount.balance:', fromAccount.balance)
    var message

    if (!opts.skipBalance && new BN(fromAccount.balance).lt(tx.getUpfrontCost())) {
      log.trace('runTx.js runCall SENDER DOESNT HAVE ENOUGH FUNDS.')
      ...
      log.trace('runTx.js runCall TX HAS WRONG NONCE.')
    ...
    log.trace('runTx.js runCall setting fromAccount balance.')
    ...
    log.trace('runTx.js runCall cache.put fromAccount done.')
    ...
    function parseResults (err, _results) {
      log.trace('runTx.js runCall parseResults...')
      ...
      log.trace('runTx.js current miner balance:', minerAccount.balance.toString('hex'))
      log.trace('runTx.js giving miner amount:', results.amountSpent)
      ...
      if (!(new BN(minerAccount.balance).isZero())) {
        log.trace('runTx.js setting miner account at address ' + block.header.coinbase.toString('hex') + ':', minerAccount)
      ...
      function () {
        ...
        log.trace('runTx.js runCall parseResults done. calling callback...')
        cb()
      })
    }
  }
