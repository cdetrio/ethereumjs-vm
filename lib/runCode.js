...
let opcodeTimes = []
const sumTimes = (accumulator, currentValue) => accumulator + currentValue;
...
module.exports = function (opts, cb) {
  ...
  function iterateVm (done) {
    ...
    t_end_op = performance.now()
    if (t_start_op) {
      let runOpTime = t_end_op - t_start_op
      // console.log("runBlock.js call to runTx took " + (tx_end - tx_start).toFixed(2) + " milliseconds.")
      opcodeTimes.push(runOpTime)
      console.log("runCode.js runOp " + lastOpName + " took " + runOpTime.toFixed(2) + " ms")
    }
    t_start_op = performance.now()
    lastOpName = opName
  ...
  function parseVmResults (err) {
    ...
    const totalTime = opcodeTimes.reduce(sumTimes)
    console.log('runCode.js parseVmResults totalTime ' + totalTime.toFixed(2) + ' ms.')
