...
const log = require('loglevel').getLogger('runBlock')
log.setLevel('silent')
...
module.exports = function (opts, cb) {
  ...
  const validateStateRoot = false // turn off for redis PoC
  ...
  function populateCache (cb) {
    console.log('---------- runBlock.js doing populateCache  ----------')
    ...
    if (argv.debug) {
      self.populateCache(accounts, function() { 
        console.log('calling processTransactions in 10 seconds...')
  ...
  function processTransactions (cb) {
    console.log('---------- runBlock.js populateCache done ----------')
    console.log('---------- runBlock.js processing txs...  ----------')
    var validReceiptCount = 0

    async.eachSeries(block.transactions, processTx, cb)

    function processTx (tx, cb) {
      const tx_start = performance.now()
      ...
      function parseTxResult (err, result) {
        const tx_end = performance.now()
        console.log("runBlock.js call to runTx took " + (tx_end - tx_start).toFixed(2) + " milliseconds.")
        console.log("runBlock.js parseTxResult err:", err)
  ...
  function parseBlockResults (err) {
    if (argv.debug) {
      console.log('calling parseBlockResults in 10 seconds...')
      setTimeout(function() { parseBlockResultsActual(err) }, 10000)
    } else {
      if (err) {
        return cb(err)
      }
  ...
  function parseBlockResultsActual (err) {
    console.log('---------- runBlock.js txs processed  ----------')
    if (err) {
      self.trie.revert()
      cb(err)
      return
    ...
    self.trie.commit(function (err) {
      self.stateManager.cache.flush(function () {
        ...
        console.log("Call to trie.commit and cache.flush took " + (t1 - t0).toFixed(2) + " milliseconds.")
        if (validateStateRoot) {
          console.log('runBlock.js parseBlockResults validating state root..')
  ...
  function payOmmersAndMiner () {
    ...
    log.trace('runBlock.js current miner balance:', minerAccount.balance.toString('hex'))
    log.trace('runBlock.js give miner block reward:', minerReward)
    minerAccount.balance = new BN(minerAccount.balance)
      .add(minerReward)
      .add(totalNiblingReward)
    log.trace('runBlock.js payOmmersAndMiner calling cache.put..')
  ...
  function rewardOmmer (ommer) {
    ...
    log.trace('runBlock.js rewardOmmer calling cache.put..')
