...
const log = require('loglevel').getLogger('opFns')
log.setLevel('silent')
...
module.exports = {
  ...
  SLOAD: function (key, runState, cb) {
    ...
    log.debug('opFns.js SLOAD getContractStorage key:', key)
    stateManager.getContractStorage(runState.address, key, function (err, value) {
      log.debug('opFns.js SLOAD getContractStorage callback err:', err)
      log.debug('opFns.js SLOAD getContractStorage callback key:', key)
      log.debug('opFns.js SLOAD getContractStorage callback value:', value)
      if (err) return cb(err)
      //value = value.length ? new BN(value) : new BN(0)
      value = value.length ? new BN(value, 16) : new BN(0)
      cb(null, value)
    })
  },
  SSTORE: function (key, val, runState, cb) {
    ...
    log.debug('opFns.js SSTORE getContractStorage key:', key)
    stateManager.getContractStorage(runState.address, key, function (err, found) {
      log.debug('opFns.js SSTORE getContractStorage callback key:', key)
      log.debug('opFns.js SSTORE getContractStorage callback err:', err)
      log.debug('opFns.js SSTORE getContractStorage callback found value:', found)
      //log.debug('opFns.js SSTORE getContractStorage callback result found:', found)
      if (err) return cb(err)
      try {
        if (value.length === 0 && !found.length) {
          log.debug('opFns.js SSTORE getContractStorage callback got result:', found)
          log.debug('opFns.js SSTORE getContractStorage callback charging 5k gas to set already empty storage slot to empty..')
          subGas(runState, new BN(fees.sstoreResetGas.v))
        } else if (value.length === 0 && found.length) {
          log.debug('opFns.js SSTORE getContractStorage callback got result:', found)
          log.debug('opFns.js SSTORE getContractStorage callback refunding 15k gas to delete..')
          subGas(runState, new BN(fees.sstoreResetGas.v))
          runState.gasRefund.iaddn(fees.sstoreRefundGas.v)
        } else if (value.length !== 0 && !found.length) {
          log.debug('opFns.js SSTORE getContractStorage callback got result:', found)
          log.debug('opFns.js SSTORE getContractStorage callback charging 20k gas to add..')
          subGas(runState, new BN(fees.sstoreSetGas.v))
        } else if (value.length !== 0 && found.length) {
          log.debug('opFns.js SSTORE getContractStorage callback got result:', found)
          log.debug('opFns.js SSTORE getContractStorage callback charging 5k gas to modify..')
          subGas(runState, new BN(fees.sstoreResetGas.v))
        }
      } catch (e) {
        cb(e.error)
      ...
      log.debug('opFns.js SSTORE getContractStorage callback calling putContractStorage key, value:', key, value)
      stateManager.putContractStorage(address, key, value, function (err) {
        log.debug('opFns.js SSTORE getContractStorage called putContractStorage callback err:', err)
        if (err) return cb(err)
...
function makeCall (runState, callOptions, localOpts, cb) {
  ...
  if (runState.depth >= fees.stackLimit.v || (callOptions.delegatecall !== true && new BN(runState.contract.balance).lt(callOptions.value))) {
    ...
    log.trace('opFns.js makeCall calling cache.put..')
