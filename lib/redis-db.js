...
async function setStorageSortedSet(accountAddr, storageObj) {
  console.log('running setStorageSortedSet')
  ...
  for (let preimageKey in storageObj) {
    console.log('preimageKey:', preimageKey)
    ...
    console.log('zadd elemString:', elemString)
...
Db.prototype.setAccountStorage = function (address, key, value, cb) {
  console.log('db.setAccountStorage address:', address)
  console.log('db.setAccountStorage key:', key)
  console.log('db.setAccountStorage value:', value)
  ...
  if (typeof key !== 'string') {
    console.log('db.setAccountstorage key SHOULD be string!:', key)
    key = key.toString('hex')
    
  }

  if (key.substr(0,2) === '0x') {
    console.log('db.setAccountstorage address, key:', address, key)
    throw(new Error('redis-db.js setAccountStorage key should not have 0x!'))
  }

  if (typeof value !== 'string') {
    console.log('db.setAccountstorage value SHOULD be string!:', value)
    value = value.toString('hex')
  }


  const redisAccountStorageKey = 'storage:' + address
  console.log('redisAccountStorageKey:', redisAccountStorageKey)
  const score = 0 // lexicographic ordering by elemString
  ...
  console.log('redis-db.js setAccountStorage zremMin, zremMax:', zremMin, zremMax)
  ...
  if (value !== '') {
    ...
    console.log('redis-db.js setAccountStorage elemString:', elemString)
...
Db.prototype.getAccountStorage = async function (address, key, cb) {
  ...
  if (typeof key !== 'string') {
    console.log('redis-db.js getAccountStorage key SHOULD BE string!:', key)
    key = key.toString('hex')
  }
  // ZSCAN storage:b94f5374fce5edbc8e2a8697c15331677e6ebf0b 0

  console.log('redis-db.js getAccountStorage. address:', address)
  console.log('redis-db.js getAccountStorage. key:', key)
  ...
  let hashedKey = utils.sha3('0x' + key).toString('hex')
  console.log('redis-db.js getAccountStorage hashedKey:', hashedKey)
  ...
  console.log('redis-db.js getAccountStorage zMin, zMax:', zMin, zMax)
  ...
  console.log('redis-db.js getAccountStorage result for zmin, zmax:', result, zMin, zMax)

  if (result.length === 0) {
    console.log('redis-db.js getAccountStorage returned null. returning:', '')
    //cb(null, rlp.encode(null))
    cb(null, '')
  }  else {
    const valEncoded = result[0].substr(130)
    console.log('redis-db.js getAccountStorage returned valEncoded:', valEncoded)
    const val = rlp.decode('0x' + valEncoded).toString('hex')
    console.log('redis-db.js getAccountStorage returning val:', val)
    cb(null, val)
  }
...
Db.prototype.clear = async function (cb) {
  ...
  */
  ...
  console.log('redis-db.js clear scan result:', result)
